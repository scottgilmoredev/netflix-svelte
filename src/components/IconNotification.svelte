<script lang="ts">
  /**
   * IconNotification Component
   *
   * @component
   * @description A bell icon used for notifications
   *
   * @prop {string} [width="24"] - Width of the icon in pixels
   * @prop {string} [height="24"] - Height of the icon in pixels
   * @prop {string} [className=""] - Additional CSS classes to apply to the SVG
   * @prop {number} [notificationCount=0] - Number of notifications to display in the badge
   * @prop {boolean} [showBadge=false] - Whether to show the notification badge
   */

  /**
   * Props for the IconNotification component
   *
   * @typedef {Object} IconNotificationProps
   * @property {string} [width="24"] - Width of the icon in pixels
   * @property {string} [height="24"] - Height of the icon in pixels
   * @property {string} [className=""] - Additional CSS classes to apply to the SVG
   * @property {number} [notificationCount=0] - Number of notifications to display in the badge
   * @property {boolean} [showBadge=false] - Whether to show the notification badge
   */
  interface IconNotificationProps {
    width?: string;
    height?: string;
    className?: string;
    notificationCount: number;
    showBadge?: boolean;
  }

  export let width: IconNotificationProps['width'] = '24';
  export let height: IconNotificationProps['height'] = '24';
  export let className: IconNotificationProps['className'] = '';
  export let notificationCount: IconNotificationProps['notificationCount'] = 0;
  export let showBadge: IconNotificationProps['showBadge'] = false;
</script>

<span class="nav__notification">
  <svg
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    role="img"
    viewBox="0 0 24 24"
    {width}
    {height}
    data-icon="BellStandard"
    aria-hidden="true"
    class={`nav__notification-icon ${className}`}
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M13.0002 4.07092C16.3924 4.55624 19 7.4736 19 11V15.2538C20.0489 15.3307 21.0851 15.4245 22.1072 15.5347L21.8928 17.5232C18.7222 17.1813 15.4092 17 12 17C8.59081 17 5.27788 17.1813 2.10723 17.5232L1.89282 15.5347C2.91498 15.4245 3.95119 15.3307 5.00003 15.2538V11C5.00003 7.47345 7.60784 4.55599 11.0002 4.07086V2H13.0002V4.07092ZM17 15.1287V11C17 8.23858 14.7614 6 12 6C9.2386 6 7.00003 8.23858 7.00003 11V15.1287C8.64066 15.0437 10.3091 15 12 15C13.691 15 15.3594 15.0437 17 15.1287ZM8.62593 19.3712C8.66235 20.5173 10.1512 22 11.9996 22C13.848 22 15.3368 20.5173 15.3732 19.3712C15.3803 19.1489 15.1758 19 14.9533 19H9.0458C8.82333 19 8.61886 19.1489 8.62593 19.3712Z"
      fill="currentColor"
    />
  </svg>

  <!-- Notification badge -->
  {#if showBadge && notificationCount > 0}
    <span class="nav__notification-pill">{notificationCount}</span>
  {/if}
</span>

<style>
  .nav__notification {
    position: relative;
    display: inline-block;
  }

  /* Notification count badge - small red circle with number */
  .nav__notification-pill {
    background-color: #b9090b;
    border: none;
    border-radius: 1000px;
    color: #fff;
    display: inline-block;
    font-size: 0.5em;
    font-weight: 500;
    line-height: 1;
    min-width: 1em;
    padding: 0.3em;
    position: absolute;
    right: -0.5em;
    text-align: center;
    top: -0.75em;
    z-index: 2;
  }
</style>
